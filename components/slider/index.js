  ...
  return (
    <Swiper spaceBetween={30}
      centeredSlides={true} autoplay={{
        ...
        "clickable": true
      }} loop={true} className="slider h-full md:h-96">
        ...
        <SwiperSlide key={slide.id}>
          <div className="slide-content">
            ...
            <div className="slide-item z-10 absolute text-left top-0 p-10 md:p-12 md:top-12 ">
              <h1 className="text-black font-bold text-xl slide-title md:text-7xl">{slide.title}</h1>
              <p className="slide-slogan text-xs md:text-xl">{slide.slogan}</p>
              <br />
              <Link href={slide.link}>
                <a className="slide-btn border-b-4 border-yellow-600 text-xs md:text-base">SHOP NOW</a>
              </Link>
            </div>
          </div>
        </SwiperSlide>
